...
class XmlReferenceContributor extends PsiReferenceContributor {
    ...
    void registerReferenceProviders(@NotNull PsiReferenceRegistrar registrar) {
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_URI_CALL), new ControllerRequestReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_RESPONSE_CALL), new ControllerViewReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_ENTITY_CALL), new EntityReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_SERVICE_DEF_CALL), new ServiceReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_LABEL_CALL), new UiLabelReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_FORM_CALL), new FormReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_FILE_CALL), new FileReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_SCREEN_CALL), new ScreenReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_MENU_CALL), new MenuReferenceProvider()
        )
        registrar.registerReferenceProvider(XmlPatterns.xmlAttributeValue()
                .withParent(OfbizPatterns.XML_JAVA_EVENT_CALL), new JavaMethodReferenceProvider()
        )
    }
}
